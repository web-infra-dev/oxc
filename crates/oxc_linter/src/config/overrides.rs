...
mod test {
    ...
    fn test_parsing_globals() {
        ...
        assert_eq!(config.globals.unwrap().get_copied("Foo").unwrap(), GlobalValue::Readonly);
    }
    ...
    fn test_parsing_env() {
        ...
        assert!(config.env.clone().unwrap().contains("es2022"));
        assert!(!config.env.unwrap().contains("es2023"));
    }
}
